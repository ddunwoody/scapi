...
void make_LSSS(int Access[],int rows,int cols,int **LSSS,int *attr)
{
	int i,j,k,z,m,d,n,t;
	node Fz,root(Access);
...
// simple, memory efficient, recursive routine in C++
// to calculate determinant of an nxn square matrix 
...
int determinant(int n,int *matrix[])
{
	...
	for (i=0;i<n;i++)
	{
		...
		det+=sign*matrix[i][0]*determinant(n-1,minor); // recurse
		sign=-sign;
	}
	delete [] minor;
    return det;
...
// Correct results are w[i]/det, where det is return value 
...
int reduce_LSSS(int &m,int &d,int **LSSS,int *attr,int *auth,int *rows,int *w)
{
    int i,j,k,n,det,nattr=0;
...
// remove redundant columns of all zeros from LSSS
	...
	if (m<d) return 0; // unable to reconstruct
// calculate reconstruction constants
	det=omega(m,LSSS,w);
	return det;  // should be 1
}
...
// Setup
	...
	g1a=pfc.mult(g1,alpha);
	g2a=pfc.mult(g2,alpha);
...
// decrypt - and apply some optimisations e.g. e(A,B)*e(A,C) = e(A,B+C)
	...
	det=reduce_LSSS(m,d,LSSS,attr,auth,rows,w);
